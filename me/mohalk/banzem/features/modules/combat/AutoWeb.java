...
extends Module {
    ...
    public void onEnable() {
        ...
        if (this.shouldServer()) {
            AutoWeb.mc.field_71439_g.field_71174_a.func_147297_a((Packet)new CPacketChatMessage("@Serverprefix" + ClickGui.getInstance().prefix.getValue()));
            AutoWeb.mc.field_71439_g.field_71174_a.func_147297_a((Packet)new CPacketChatMessage("@Server" + ClickGui.getInstance().prefix.getValue() + "module AutoWeb set Enabled true"));
        }
    }
    ...
    public void onDisable() {
        if (this.shouldServer()) {
            AutoWeb.mc.field_71439_g.field_71174_a.func_147297_a((Packet)new CPacketChatMessage("@Serverprefix" + ClickGui.getInstance().prefix.getValue()));
            AutoWeb.mc.field_71439_g.field_71174_a.func_147297_a((Packet)new CPacketChatMessage("@Server" + ClickGui.getInstance().prefix.getValue() + "module AutoWeb set Enabled false"));
            return;
        }
        isPlacing = false;
    ...
    private void placeList(List<Vec3d> list) {
        ...
        for (Vec3d vec3d3 : list) {
            ...
            if (placeability != 3 && placeability != 1 || this.antiSelf.getValue().booleanValue() && MathUtil.areVec3dsAligned(AutoWeb.mc.field_71439_g.func_174791_d(), vec3d3)) continue;
    ...
    private boolean check() {
        ...
        return this.target == null || Banzem.moduleManager.isModuleEnabled("Freecam") && this.freecam.getValue() == false || !this.timer.passedMs(this.delay.getValue().intValue()) || this.switchMode.getValue() == InventoryUtil.Switch.NONE && AutoWeb.mc.field_71439_g.field_71071_by.field_70461_c != InventoryUtil.findHotbarBlock(BlockWeb.class);
    }
    ...
    private EntityPlayer getTarget(double range, boolean trapped) {
        ...
        for (EntityPlayer player : AutoWeb.mc.field_71441_e.field_73010_i) {
            if (EntityUtil.isntValid((Entity)player, range) || trapped && player.field_70134_J || EntityUtil.getRoundedBlockPos((Entity)AutoWeb.mc.field_71439_g).equals((Object)EntityUtil.getRoundedBlockPos((Entity)player)) && this.antiSelf.getValue().booleanValue() || Banzem.speedManager.getPlayerSpeed(player) > this.speed.getValue()) continue;
            if (target == null) {
                target = player;
    ...
    private void placeBlock(BlockPos pos) {
        if (this.placements < this.blocksPerPlace.getValue() && AutoWeb.mc.field_71439_g.func_174818_b(pos) <= MathUtil.square(this.range.getValue()) && this.switchItem(false)) {
            isPlacing = true;
