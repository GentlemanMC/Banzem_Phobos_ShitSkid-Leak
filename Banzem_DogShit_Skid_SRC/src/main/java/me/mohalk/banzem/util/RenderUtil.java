...
public class RenderUtil implements Util
{
    ...
    public static void drawWaypointImage(final BlockPos pos, final GLUProjection.Projection projection, final Color color, final String name, final boolean rectangle, final Color rectangleColor) {
        ...
        final float textWidth = (float)Banzem.textManager.getStringWidth(text);
        Banzem.textManager.drawString(text, -(textWidth / 2.0f), -(Banzem.textManager.getFontHeight() / 2.0f) + Banzem.textManager.getFontHeight() / 2.0f, color.getRGB(), false);
        GlStateManager.translate(-projection.getX(), -projection.getY(), 0.0);
    ...
    public static void drawSexyBoxBanzemIsRetardedFuckYouESP(final AxisAlignedBB a, final Color boxColor, final Color outlineColor, final float lineWidth, final boolean outline, final boolean box, final boolean colorSync, float alpha, final float scale, final float slab) {
    ...
    public static void drawText(final AxisAlignedBB pos, final String text) {
        ...
        GlStateManager.disableDepth();
        GlStateManager.translate(-(Banzem.textManager.getStringWidth(text) / 2.0), 0.0, 0.0);
    ...
    public static void drawProjectedText(final GLUProjection.Projection projection, final float addX, final float addY, final String text, final Color color, final boolean shadow) {
        if (projection.getType() == GLUProjection.Projection.Type.INSIDE) {
            GlStateManager.pushMatrix();
            GlStateManager.translate(projection.getX(), projection.getY(), 0.0);
            Banzem.textManager.drawString(text, -(Banzem.textManager.getStringWidth(text) / 2.0f) + addX, addY, color.getRGB(), shadow);
            GlStateManager.translate(-projection.getX(), -projection.getY(), 0.0);
            GlStateManager.popMatrix();
        }
    }
