...
public class StringButton extends Button
{
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        if (this.isListening) {
            Banzem.textManager.drawStringWithShadow(this.currentString.getString() + Banzem.textManager.getIdleSign(), this.x + 2.3f, this.y - 1.7f - PhobosGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
        else {
            Banzem.textManager.drawStringWithShadow((this.setting.shouldRenderName() ? (this.setting.getName() + " §7") : "") + this.setting.getValue(), this.x + 2.3f, this.y - 1.7f - PhobosGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
