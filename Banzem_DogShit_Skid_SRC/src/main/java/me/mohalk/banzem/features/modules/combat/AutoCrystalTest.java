...
public class AutoCrystalTest extends Module
{
    ...
    public AutoCrystalTest() {
        super("AutoCrystalTest", "Best CA on the market", Category.COMBAT, true, false, false);
        ...
        this.antiNaked = (Setting<Boolean>)this.register(new Setting("AntiNaked", Boolean.TRUE, v -> this.setting.getValue() == Settings.MISC));
    ...
    public String getDisplayInfo() {
        ...
        if (AutoCrystalTest.target != null) {
            return AutoCrystalTest.target.getName();
        }
        return null;
    }
    ...
    public void onPacketSend(final PacketEvent.Send event) {
        ...
        if (event.getStage() == 0 && event.getPacket() instanceof CPacketUseEntity && (packet3 = event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet3.getEntityFromWorld((World)AutoCrystalTest.mc.world) instanceof EntityEnderCrystal) {
            pos = Objects.requireNonNull(packet3.getEntityFromWorld((World)AutoCrystalTest.mc.world)).getPosition();
            if (this.removeAfterAttack.getValue()) {
                Objects.requireNonNull(packet3.getEntityFromWorld((World)AutoCrystalTest.mc.world)).setDead();
                AutoCrystalTest.mc.world.removeEntityFromWorld(packet3.entityId);
            }
        }
        if (event.getStage() == 0 && event.getPacket() instanceof CPacketUseEntity && (packet3 = event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet3.getEntityFromWorld((World)AutoCrystalTest.mc.world) instanceof EntityEnderCrystal) {
            final EntityEnderCrystal crystal = (EntityEnderCrystal)packet3.getEntityFromWorld((World)AutoCrystalTest.mc.world);
        ...
        if (!this.justRender.getValue() && this.switchTimer.passedMs(this.switchCooldown.getValue()) && this.explode.getValue() && this.instant.getValue() && event.getPacket() instanceof SPacketSpawnObject && (this.syncedCrystalPos == null || !this.syncedFeetPlace.getValue() || this.damageSync.getValue() == DamageSync.NONE)) {
            ...
            if (packet2.getType() == 51 && AutoCrystalTest.mc.player.getDistanceSq(pos = new BlockPos(packet2.getX(), packet2.getY(), packet2.getZ())) + this.predictOffset.getValue() <= MathUtil.square(this.breakRange.getValue()) && (this.instantTimer.getValue() == PredictTimer.NONE || (this.instantTimer.getValue() == PredictTimer.BREAK && this.breakTimer.passedMs(this.breakDelay.getValue())) || (this.instantTimer.getValue() == PredictTimer.PREDICT && this.predictTimer.passedMs(this.predictDelay.getValue())))) {
                ...
                if (this.predictFriendDmg.getValue() && (this.antiFriendPop.getValue() == AntiFriendPop.BREAK || this.antiFriendPop.getValue() == AntiFriendPop.ALL) && this.isRightThread()) {
                    for (final EntityPlayer friend : AutoCrystalTest.mc.world.playerEntities) {
                        if (friend != null && !AutoCrystalTest.mc.player.equals((Object)friend) && friend.getDistanceSq(pos) <= MathUtil.square(this.range.getValue() + this.placeRange.getValue()) && Banzem.friendManager.isFriend(friend)) {
                ...
                if (AutoCrystalTest.placedPos.contains(pos.down())) {
                    Label_0621: {
                        if (this.isRightThread() && this.superSafe.getValue()) {
                            ...
                            if ((selfDamage = DamageUtil.calculateDamage(pos, (Entity)AutoCrystalTest.mc.player)) - 0.5 <= EntityUtil.getHealth((Entity)AutoCrystalTest.mc.player)) {
                    ...
                    if (DamageUtil.canTakeDamage(this.suicide.getValue())) {
                        selfDamage = DamageUtil.calculateDamage(pos, (Entity)AutoCrystalTest.mc.player);
                    }
                    if (selfDamage + 0.5 < EntityUtil.getHealth((Entity)AutoCrystalTest.mc.player) && selfDamage <= this.maxSelfBreak.getValue()) {
                        for (final EntityPlayer player : AutoCrystalTest.mc.world.playerEntities) {
            ...
            for (final int id : packet4.getEntityIDs()) {
                final Entity entity = AutoCrystalTest.mc.world.getEntityByID(id);
            ...
            if (packet5.getOpCode() == 35 && packet5.getEntity((World)AutoCrystalTest.mc.world) instanceof EntityPlayer) {
                this.totemPops.put((EntityPlayer)packet5.getEntity((World)AutoCrystalTest.mc.world), new Timer().reset());
            }
        }
        else {
            ...
            if (event.getPacket() instanceof SPacketSoundEffect && (packet6 = event.getPacket()).getCategory() == SoundCategory.BLOCKS && packet6.getSound() == SoundEvents.ENTITY_GENERIC_EXPLODE) {
                ...
                if (this.threadMode.getValue() == ThreadMode.SOUND && this.isRightThread() && AutoCrystalTest.mc.player != null && AutoCrystalTest.mc.player.getDistanceSq(pos) < MathUtil.square(this.soundPlayer.getValue())) {
    ...
    private boolean predictSlowBreak(final BlockPos pos) {
        return this.antiCommit.getValue() && AutoCrystalTest.lowDmgPos.remove(pos) && this.shouldSlowBreak(false);
    }
    
    private boolean isRightThread() {
        return AutoCrystalTest.mc.isCallingFromMinecraftThread() || (!Banzem.eventManager.ticksOngoing() && !this.threadOngoing.get());
    }
    ...
    private void attackCrystalPredict(final int entityID, final BlockPos pos) {
        ...
        AutoCrystalTest.mc.player.connection.sendPacket((Packet)attackPacket);
        if (this.breakSwing.getValue()) {
            AutoCrystalTest.mc.player.connection.sendPacket((Packet)new CPacketAnimation(EnumHand.MAIN_HAND));
        }
    ...
    private void removePos(final BlockPos pos) {
        if (this.damageSync.getValue() == DamageSync.PLACE) {
            if (AutoCrystalTest.placedPos.remove(pos)) {
                this.posConfirmed = true;
            }
        }
        else if (this.damageSync.getValue() == DamageSync.BREAK && AutoCrystalTest.brokenPos.remove(pos)) {
            this.posConfirmed = true;
        }
    }
    ...
    public void onRender3D(final Render3DEvent event) {
        ...
        if ((this.offHand || this.mainHand || this.switchMode.getValue() == Switch.CALC) && this.renderPos != null && (this.box.getValue() || this.outline.getValue())) {
            ...
            if (this.renderMode.getValue() == RenderMode.STATIC) {
                RenderUtil.drawSexyBoxBanzemIsRetardedFuckYouESP(new AxisAlignedBB(this.renderPos), boxC, outlineC, this.lineWidth.getValue(), this.outline.getValue(), this.box.getValue(), this.colorSync.getValue(), 1.0f, 1.0f, this.slabHeight.getValue());
            }
            if (this.renderMode.getValue() == RenderMode.GLIDE) {
                if (this.lastRenderPos == null || AutoCrystalTest.mc.player.getDistance(this.renderBB.minX, this.renderBB.minY, this.renderBB.minZ) > this.range.getValue()) {
                ...
                RenderUtil.drawSexyBoxBanzemIsRetardedFuckYouESP(this.renderBB = this.renderBB.offset(xDiff * multiplier, yDiff * multiplier, zDiff * multiplier), boxC, outlineC, this.lineWidth.getValue(), this.outline.getValue(), this.box.getValue(), this.colorSync.getValue(), 1.0f, 1.0f, this.slabHeight.getValue());
        ...
        if (this.renderMode.getValue() == RenderMode.FADE) {
            ...
            this.positions.forEach(pos -> {
                ...
                RenderUtil.drawSexyBoxBanzemIsRetardedFuckYouESP(new AxisAlignedBB(pos.getPos()), boxColor, outlineColor, this.lineWidth.getValue(), this.outline.getValue(), this.box.getValue(), this.colorSync.getValue(), ((boolean)this.fadeFactor.getValue()) ? factor[0] : 1.0f, ((boolean)this.scaleFactor.getValue()) ? factor[0] : 1.0f, ((boolean)this.slabFactor.getValue()) ? factor[0] : 1.0f);
                pos.setRenderTime(pos.getRenderTime() + 50.0f);
                return;
            });
            this.positions.removeIf(pos -> pos.getRenderTime() >= this.duration.getValue() || AutoCrystalTest.mc.world.isAirBlock(pos.getPos()) || !AutoCrystalTest.mc.world.isAirBlock(pos.getPos().offset(EnumFacing.UP)));
    ...
    public void onKeyInput(final InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState() && !(AutoCrystalTest.mc.currentScreen instanceof PhobosGui) && this.switchBind.getValue().getKey() == Keyboard.getEventKey()) {
    ...
    private void postProcessBreak() {
        while (!this.packetUseEntities.isEmpty()) {
            ...
            AutoCrystalTest.mc.player.connection.sendPacket((Packet)packet);
            if (this.breakSwing.getValue()) {
                AutoCrystalTest.mc.player.swingArm(EnumHand.MAIN_HAND);
            }
            this.breakTimer.reset();
        }
    }
    ...
    private void handleWhile() {
        if (this.thread == null || this.thread.isInterrupted() || !this.thread.isAlive() || (this.syncroTimer.passedMs(this.syncThreads.getValue()) && this.syncThreadBool.getValue())) {
            if (this.thread == null) {
                this.thread = new Thread(RAutoCrystalTest.getInstance(this));
            }
    ...
    private ScheduledExecutorService getExecutor() {
        ...
        service.scheduleAtFixedRate(RAutoCrystalTest.getInstance(this), 0L, this.threadDelay.getValue(), TimeUnit.MILLISECONDS);
        return service;
    }
    
    public void doAutoCrystalTest() {
        if (this.check()) {
            switch (this.logic.getValue()) {
                case PLACEBREAK: {
                    this.placeCrystal();
    ...
    private boolean check() {
        ...
        this.mainHand = (AutoCrystalTest.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL);
        ...
        this.offHand = (AutoCrystalTest.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL);
        this.currentDamage = 0.0;
        this.placePos = null;
        if (this.lastSlot != AutoCrystalTest.mc.player.inventory.currentItem || AutoTrap.isPlacing || Surround.isPlacing) {
            this.lastSlot = AutoCrystalTest.mc.player.inventory.currentItem;
        ...
        if ((!this.offHand && !this.mainHand && this.switchMode.getValue() == Switch.BREAKSLOT && !this.switching) || !DamageUtil.canBreakWeakness((EntityPlayer)AutoCrystalTest.mc.player) || !this.switchTimer.passedMs(this.switchCooldown.getValue())) {
            this.renderPos = null;
            AutoCrystalTest.target = null;
            return this.rotating = false;
        }
        if (this.mineSwitch.getValue() && Mouse.isButtonDown(0) && (this.switching || this.autoSwitch.getValue() == AutoSwitch.ALWAYS) && Mouse.isButtonDown(1) && AutoCrystalTest.mc.player.getHeldItemMainhand().getItem() instanceof ItemPickaxe) {
            this.switchItem();
        }
        this.mapCrystals();
    ...
    private void mapCrystals() {
        ...
        for (final Entity entity : AutoCrystalTest.mc.world.loadedEntityList) {
            if (!entity.isDead && entity instanceof EntityEnderCrystal) {
                ...
                if (this.syncedFeetPlace.getValue() && entity.getPosition().down().equals((Object)this.syncedCrystalPos) && this.damageSync.getValue() != DamageSync.NONE) {
                    ...
                    float selfDamage = -1.0f;
                    if (DamageUtil.canTakeDamage(this.suicide.getValue())) {
                        selfDamage = DamageUtil.calculateDamage(entity, (Entity)AutoCrystalTest.mc.player);
                    }
                    if (selfDamage + 0.5 < EntityUtil.getHealth((Entity)AutoCrystalTest.mc.player) && selfDamage <= this.maxSelfBreak.getValue()) {
                        ...
                        for (final EntityPlayer player : AutoCrystalTest.mc.world.playerEntities) {
                            ...
                            if (EntityUtil.isValid((Entity)player, this.range.getValue() + this.breakRange.getValue())) {
                                ...
                                if ((this.antiFriendPop.getValue() != AntiFriendPop.BREAK && this.antiFriendPop.getValue() != AntiFriendPop.ALL) || !Banzem.friendManager.isFriend(player.getName())) {
                                    continue;
                                }
        ...
        if (this.webAttack.getValue() && this.webPos != null) {
            if (AutoCrystalTest.mc.player.getDistanceSq(this.webPos.up()) > MathUtil.square(this.breakRange.getValue())) {
                this.webPos = null;
            }
            else {
                for (final Object entity : AutoCrystalTest.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(this.webPos.up()))) {
                    if (!(entity instanceof EntityEnderCrystal)) {
                        continue;
                    }
                    this.attackList.add((Entity) entity);
                    this.efficientTarget = (Entity) entity;
                    this.webPos = null;
                    this.lastDamage = 0.5;
                    return;
                }
        ...
        if (this.shouldSlowBreak(true) && maxDamage < this.minDamage.getValue() && (AutoCrystalTest.target == null || EntityUtil.getHealth((Entity)AutoCrystalTest.target) > this.facePlace.getValue() || (!this.breakTimer.passedMs(this.facePlaceSpeed.getValue()) && this.slowFaceBreak.getValue() && Mouse.isButtonDown(0) && this.holdFacePlace.getValue() && this.holdFaceBreak.getValue()))) {
            this.efficientTarget = null;
            return;
        }
        if (this.packets.getValue() == 1) {
    ...
    private void placeCrystal() {
        ...
        if (this.placeTimer.passedMs(this.placeDelay.getValue()) && this.place.getValue() && (this.offHand || this.mainHand || this.switchMode.getValue() == Switch.CALC || (this.switchMode.getValue() == Switch.BREAKSLOT && this.switching))) {
            ...
            if (AutoCrystalTest.target != null && this.placePos != null) {
                if (!this.offHand && !this.mainHand && this.autoSwitch.getValue() != AutoSwitch.NONE && (this.currentDamage > this.minDamage.getValue() || (this.lethalSwitch.getValue() && EntityUtil.getHealth((Entity)AutoCrystalTest.target) <= this.facePlace.getValue())) && !this.switchItem()) {
                    return;
                }
                if (this.currentDamage < this.minDamage.getValue() && this.limitFacePlace.getValue()) {
                    crystalLimit = 1;
                }
                if (this.currentDamage >= this.minMinDmg.getValue() && (this.offHand || this.mainHand || this.autoSwitch.getValue() != AutoSwitch.NONE) && (this.crystalCount < crystalLimit || (this.antiSurround.getValue() && this.lastPos != null && this.lastPos.equals((Object)this.placePos))) && (this.currentDamage > this.minDamage.getValue() || this.minDmgCount < crystalLimit) && this.currentDamage >= 1.0 && (DamageUtil.isArmorLow(AutoCrystalTest.target, this.minArmor.getValue()) || EntityUtil.getHealth((Entity)AutoCrystalTest.target) <= this.facePlace.getValue() || this.currentDamage > this.minDamage.getValue() || this.shouldHoldFacePlace())) {
                    final float damageOffset = (this.damageSync.getValue() == DamageSync.BREAK) ? (this.dropOff.getValue() - 5.0f) : 0.0f;
                    boolean syncflag = false;
                    if (this.syncedFeetPlace.getValue() && this.placePos.equals((Object)this.lastPos) && this.isEligableForFeetSync(AutoCrystalTest.target, this.placePos) && !this.syncTimer.passedMs(this.damageSyncTime.getValue()) && AutoCrystalTest.target.equals((Object)this.currentSyncTarget) && AutoCrystalTest.target.getPosition().equals((Object)this.syncedPlayerPos) && this.damageSync.getValue() != DamageSync.NONE) {
                    ...
                    if (syncflag || this.currentDamage - damageOffset > this.lastDamage || this.syncTimer.passedMs(this.damageSyncTime.getValue()) || this.damageSync.getValue() == DamageSync.NONE) {
                        ...
                        if (this.switchItem()) {
                            ...
                            if (this.addTolowDmg || (this.actualSlowBreak.getValue() && this.currentDamage < this.minDamage.getValue())) {
                                AutoCrystalTest.lowDmgPos.add(this.placePos);
                            }
    ...
    private boolean doSwitch() {
        if (!this.offhandSwitch.getValue()) {
            if (AutoCrystalTest.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL) {
                this.mainHand = false;
            }
            else {
    ...
    private void calculateDamage(final EntityPlayer targettedPlayer) {
        ...
        if (this.webAttack.getValue() && targettedPlayer != null && AutoCrystalTest.mc.world.getBlockState(playerPos = new BlockPos(targettedPlayer.getPositionVector())).getBlock() == Blocks.WEB) {
            setToAir = playerPos;
            state = AutoCrystalTest.mc.world.getBlockState(playerPos);
            AutoCrystalTest.mc.world.setBlockToAir(playerPos);
        }
        for (final BlockPos pos : BlockUtil.possiblePlacePositions(this.placeRange.getValue(), this.antiSurround.getValue(), this.oneDot15.getValue())) {
            if (!BlockUtil.rayTracePlaceCheck(pos, (this.raytrace.getValue() == Raytrace.PLACE || this.raytrace.getValue() == Raytrace.FULL) && AutoCrystalTest.mc.player.getDistanceSq(pos) > MathUtil.square(this.placetrace.getValue()), 1.0f)) {
                continue;
            }
            float selfDamage = -1.0f;
            if (DamageUtil.canTakeDamage(this.suicide.getValue())) {
            ...
            if (selfDamage + 0.5 >= EntityUtil.getHealth((Entity)AutoCrystalTest.mc.player)) {
                continue;
            }
            ...
            if (targettedPlayer != null) {
                ...
                for (final EntityPlayer player : AutoCrystalTest.mc.world.playerEntities) {
                    if (EntityUtil.isValid((Entity)player, this.placeRange.getValue() + this.range.getValue())) {
                        ...
                        if ((this.antiFriendPop.getValue() != AntiFriendPop.ALL && this.antiFriendPop.getValue() != AntiFriendPop.PLACE) || player == null || player.getDistanceSq(pos) > MathUtil.square(this.range.getValue() + this.placeRange.getValue()) || !Banzem.friendManager.isFriend(player)) {
                            continue;
                        }
        ...
        if (setToAir != null) {
            AutoCrystalTest.mc.world.setBlockState(setToAir, state);
    ...
    private EntityPlayer getTarget(final boolean unsafe) {
        ...
        for (final EntityPlayer player : AutoCrystalTest.mc.world.playerEntities) {
            if (!EntityUtil.isntValid((Entity)player, this.placeRange.getValue() + this.range.getValue()) && (!this.antiNaked.getValue() || !DamageUtil.isNaked(player))) {
                ...
                if (currentTarget == null) {
                    ...
                    if (AutoCrystalTest.mc.player.getDistanceSq((Entity)player) >= AutoCrystalTest.mc.player.getDistanceSq((Entity)currentTarget)) {
                        continue;
                    }
                    currentTarget = player;
                }
        ...
        if (this.predictPos.getValue() && currentTarget != null) {
            ...
            final EntityOtherPlayerMP newTarget = new EntityOtherPlayerMP((World)AutoCrystalTest.mc.world, profile);
    ...
    private void doFakeSwing() {
        if (this.fakeSwing.getValue()) {
            EntityUtil.swingArmNoPacket(EnumHand.MAIN_HAND, (EntityLivingBase)AutoCrystalTest.mc.player);
        }
    }
    ...
    private void manualBreaker() {
        if (this.rotate.getValue() != Rotate.OFF && this.eventMode.getValue() != 2 && this.rotating) {
            if (this.didRotation) {
                AutoCrystalTest.mc.player.rotationPitch += (float)4.0E-4;
        ...
        if ((this.offHand || this.mainHand) && this.manual.getValue() && this.manualTimer.passedMs(this.manualBreak.getValue()) && Mouse.isButtonDown(1) && AutoCrystalTest.mc.player.getHeldItemOffhand().getItem() != Items.GOLDEN_APPLE && AutoCrystalTest.mc.player.inventory.getCurrentItem().getItem() != Items.GOLDEN_APPLE && AutoCrystalTest.mc.player.inventory.getCurrentItem().getItem() != Items.BOW && AutoCrystalTest.mc.player.inventory.getCurrentItem().getItem() != Items.EXPERIENCE_BOTTLE && (result = AutoCrystalTest.mc.objectMouseOver) != null) {
            switch (result.typeOfHit) {
                ...
                case BLOCK: {
                    final BlockPos mousePos = AutoCrystalTest.mc.objectMouseOver.getBlockPos().up();
                    for (final Entity target : AutoCrystalTest.mc.world.getEntitiesWithinAABBExcludingEntity((Entity)null, new AxisAlignedBB(mousePos))) {
    ...
    private void rotateTo(final Entity entity) {
        switch (this.rotate.getValue()) {
            ...
            case BREAK:
            case ALL: {
                final float[] angle = MathUtil.calcAngle(AutoCrystalTest.mc.player.getPositionEyes(AutoCrystalTest.mc.getRenderPartialTicks()), entity.getPositionVector());
    ...
    private void rotateToPos(final BlockPos pos) {
        switch (this.rotate.getValue()) {
            ...
            case PLACE:
            case ALL: {
                final float[] angle = MathUtil.calcAngle(AutoCrystalTest.mc.player.getPositionEyes(AutoCrystalTest.mc.getRenderPartialTicks()), new Vec3d((double)(pos.getX() + 0.5f), (double)(pos.getY() - 0.5f), (double)(pos.getZ() + 0.5f)));
                if (this.eventMode.getValue() == 2 && this.threadMode.getValue() == ThreadMode.NONE) {
                    Banzem.rotationManager.setPlayerRotations(angle[0], angle[1]);
                    break;
                }
    ...
    private boolean isValid(final Entity entity) {
        return entity != null && AutoCrystalTest.mc.player.getDistanceSq(entity) <= MathUtil.square(this.breakRange.getValue()) && (this.raytrace.getValue() == Raytrace.NONE || this.raytrace.getValue() == Raytrace.PLACE || AutoCrystalTest.mc.player.canEntityBeSeen(entity) || (!AutoCrystalTest.mc.player.canEntityBeSeen(entity) && AutoCrystalTest.mc.player.getDistanceSq(entity) <= MathUtil.square(this.breaktrace.getValue())));
    }
    ...
    static {
        AutoCrystalTest.lowDmgPos = (Set<BlockPos>)new ConcurrentSet();
        AutoCrystalTest.placedPos = new HashSet<BlockPos>();
        AutoCrystalTest.brokenPos = new HashSet<BlockPos>();
    }
    ...
    private static class RAutoCrystalTest implements Runnable
    {
        private static RAutoCrystalTest instance;
        private AutoCrystalTest autoCrystal;
        
        public static RAutoCrystalTest getInstance(final AutoCrystalTest autoCrystal) {
            if (RAutoCrystalTest.instance == null) {
                RAutoCrystalTest.instance = new RAutoCrystalTest();
                RAutoCrystalTest.instance.autoCrystal = autoCrystal;
            }
            return RAutoCrystalTest.instance;
        }
        ...
        public void run() {
            if (this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                while (this.autoCrystal.isOn() && this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                    ...
                    this.autoCrystal.doAutoCrystalTest();
