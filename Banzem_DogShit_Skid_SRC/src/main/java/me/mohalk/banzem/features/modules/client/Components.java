...
public class Components extends Module
{
    ...
    public void drawTargetHud(final float partialTicks) {
        if (this.design.getValue() == TargetHudDesign.NORMAL) {
            final EntityPlayer target = (AutoCrystal.target != null) ? AutoCrystal.target : ((Killaura.target instanceof EntityPlayer) ? (EntityPlayer) Killaura.target : getClosestEnemy());
            if (target == null) {
                return;
            }
            ...
            final Integer ping = EntityUtil.isFakePlayer(target) ? 0 : ((Util.mc.getConnection().getPlayerInfo(target.getUniqueID()) == null) ? 0 : Util.mc.getConnection().getPlayerInfo(target.getUniqueID()).getResponseTime());
            ...
            this.renderer.drawStringWithShadow("Pops: " + Banzem.totemPopManager.getTotemPops(target), (float)(this.targetHudX.getValue() + 60), (float)(this.targetHudY.getValue() + this.renderer.getFontHeight() * 2 + 30), ColorUtil.toRGBA(255, 0, 0, 255));
            GlStateManager.enableTexture2D();
            int iteration = 0;
    ...
    static {
        ...
        Components.logo = new ResourceLocation("textures/banzem.png");
    }
